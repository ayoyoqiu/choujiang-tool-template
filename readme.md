...
1. 点击"音乐配置"按钮，打开音乐配置弹窗
2. 点击"选择文件"按钮，上传音频文件（支持 MP3、WAV、OGG 等格式，建议 < 10MB）
3. 上传后可以点击"试听"按钮预览音乐
4. 点击"保存"按钮保存音乐配置
5. 配置后，点击"开始"抽奖时，音乐会自动循环播放
6. 点击"暂停"或抽奖结束时，音乐会自动停止
7. 可以点击"删除音乐"按钮删除已配置的音乐
...
- **CSS3**：样式和布局，支持响应式设计
- **JavaScript (ES6+)**：业务逻辑
- **LocalStorage**：数据持久化（名单、结果、配置、音乐、模式）
...
- **数据存储**：使用 `localStorage` 存储名单、抽奖结果、奖品配置、音乐配置、抽取模式
- **音乐播放**：使用 HTML5 Audio API 实现背景音乐循环播放，自动控制播放和停止
...
8. **音乐配置**：
   ...
   - 文件大小建议 < 10MB（受 localStorage 存储限制影响，实际可存储大小约为 3-5MB）
   - 音乐以 base64 格式存储在 localStorage 中
   - 浏览器可能阻止自动播放，需要用户先与页面交互
   - 如果存储空间不足，系统会自动提示
...
- ✅ 新增**音乐配置功能**：支持上传音频文件作为抽奖背景音乐，开始抽奖时自动循环播放，暂停时自动停止
...
- ✅ 更新**默认名单**：改为测试1-测试15，方便测试使用
- ✅ 优化**存储空间检测**：音乐上传时自动检测可用存储空间，防止存储失败
